...
def nan_missing_weights(weights : NDArray[Any], 
    ...
    '''
    If a pedestrian k was missing, all weights of the edges connected to
    k are set to NaN for the networks representing the missing time 
    windows, as well as the networks within ±tau seconds.

    Parameters
    ----------
    weights : ndarray of network weights in a given time window
        Shape (num_networks, N, N)
    x : ndarray of x positions 
    ...
    for network_ID in range(num_networks):  # [0, num_networks-1]
        ...
        missing_data_rows = np.any(
            np.isnan(x[start_frame:(end_frame+1), :]), axis=0)
...
def get_network_weights(index : NDArray[Any], 
    ...
    Parameters
    ----------
    index: ndarray of max tau indices 
        Shape : (N, N, num_frames)
    x : ndarray of x positions
        Shape (num_timepoints, N)
    y : ndarray of y positions 
        Shape (num_timepoints, N)
    heading: ndarray of headings
        Shape (num_timepoints, N)
    SAMP_FREQ: int or float
        sample frequency (in Hz)
    pruning (default = True) : bool (optional)
        whether weights should be pruned
    TAU (default = 3) : int or float (optional)
        TAU window size (in seconds)
    NTWK_WINDOW_SIZE (default = 0.5): int or float (optional)
        network window size (in seconds)
    vis_adjust (default = False) : bool (optional)
        if weights should be adjusted based on visibility
    visibility_threshold (default = 0.15) : float (optional)
    timedelay_low_threshold (default = 0.3) : float (optional)

    Returns
    -------
    weights : ndarray of network weights in the same time window.
        Shape : (num_networks, N, N)
    '''
    ## ===== init =====
    ...
    if (num_frames_last_network >= (num_frames_network/2)):
        num_networks += 1
    # get the start & end frame IDs for each network, (num_networks,)
    start_frames = np.arange(start = 0, 
                             stop = num_frames_network * (num_networks-1) + 1, 
                             step = num_frames_network).astype(int)
    ...
    for ntwk in range(num_networks):
        ...
        leading_mask = (
            index[:, :, start_frames[ntwk]:end_frames[ntwk]+1] \
            > TAU * SAMP_FREQ
        )   # (N, N, num_frames_network)
        num_frames_lead = np.sum(leading_mask, axis=2)     # per pair, (N, N)
        ## get a proportion of time i is leading j in the given network
        weights[ntwk,:, :] = (
            num_frames_lead / (end_frames[ntwk]-start_frames[ntwk]+1))

    ## ===== apply pruning (visibility & time delay) =====
    if (pruning==True):
        weights, _ = prune_with_visibility(
            weights, start_frames, end_frames, x, y, heading,
            vis_adjust=vis_adjust,threshold=visibility_threshold
        )
        weights, _ = prune_with_timedelay(
            weights, start_frames, end_frames, index, SAMP_FREQ,
            TAU=TAU, low_bound=timedelay_low_threshold
        )
        
    ## ===== change values to nan for missing values =====
    weights = nan_missing_weights(
        weights, x, SAMP_FREQ, NTWK_WINDOW_SIZE, 
        TAU=TAU
    )
...
def create_network(weights : NDArray[Any],
    ...
    Parameters
    ----------
    weights: ndarray of network weights
        Shape (N, N).
...
def plot_networks(weights : NDArray[Any],
    ...
    IDs (default = None) : list or ndarray (optional)
    order : str (optional)
        Indicates the order of weights.
        * if 'ij' (default) -> the 1st dimension of weights (rows) 
        represents pedestrian i (leaders), and the 2nd dimension 
    ...
    for ntwk in range(num_networks):
        ...
        # --- plot ---
        ax = (ax_all[ntwk%ncols] if nrows==1 
              else ax_all[ntwk//ncols, ntwk%ncols])
        # plot a network
        if num_confs==0:
            c = '#1f78b4'
        else:   # red: confederate, black: participant
            c = ['red' for _ in range(num_confs)]
            c += ['black' for _ in range(len(G.nodes()) - num_confs)]
