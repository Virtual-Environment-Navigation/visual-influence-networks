...
def get_velocity(pos : NDArray[Any], 
                 ...
                 insert_row: Literal['zeros', 'first_row'] = 'zeros'
                 ) -> NDArray[Any]:
    '''
    ...
    Parameters
    ----------
    pos : ndarray of shape (num_timepoints, N) or (num_timepoints, N, 2)
    SAMP_FREQ : sample frequency (Hz)
    same_shape (default = True) : bool (optional)
        Whether vel should be the same shape as pos.
        * if True -> a row of zeros is added at vel[0]
        * if False -> vel would be different from pos
    insert_row (default = 'zeros') : str (optional)
        Determines what values should be inserted into the first row.
        Used only if same_shape is True.
        * if 'zeros' -> a row of zeros is inserted
        * if 'first_row' -> the same values are repeated

    Returns
    -------
    vel: ndarray of the same shape as pos
        Velocities. By default, the values of the first row are zeros.
    '''
...
def get_acceleration(vel : NDArray[Any], 
    ...
    Parameters
    ----------
    vel: ndarray of shape (num_timepoints, N) or (num_timepoints, N, 2)
        Velocities. See the description of pos & vel in get_velocity()
    SAMP_FREQ : sample frequency (Hz)
    same_shape (default = True) : bool (optional)
        Whether acc should be the same shape as vel.
        * if True -> a row of zeros is added at acc[0]
        * if False -> acc would be different from vel
    insert_row (default = 'zeros') : str (optional)
        Determines what values should be inserted into the first row. 
        Used only if same_shape is True.
        * if 'zeros' -> a row of zeros is inserted
    ...
    if (same_shape==True):
        if (vel.ndim == 1):     # if pos is 1D
            ...
            acc = np.insert(acc, 0, new_value)  # add at the beginning
        else:   # if pos is 2D or more
            new_row = (np.zeros_like(acc[0]) if (insert_row == 'zeros') 
                       else acc[0])
...
def get_speed(x : NDArray[Any], 
    ...
    Parameters
    ----------
    x, y : ndarray of any shape
        X & Y positions
    SAMP_FREQ : sample frequency (Hz)

    Returns
    -------
    speed : ndarray of the same shape as x & y
        Speed
    '''
    vx = np.diff(x)
    vy = np.diff(y)
...
def get_heading(x : NDArray[Any], 
    ...
    '''
    Get speed & heading time series from time series of position (x,y). 
                  0
                  ^
                  |    
         -90  ————————> +90
    ...
    Returns
    -------
    heading : ndarray of the same shape as x & y
        Heading, where 0º is set as North direction, ranging between 
        [-180,180].
    '''
    vx = np.diff(x)
    vy = np.diff(y)
...
def get_mean_angle(angles : NDArray[Any], 
    ...
    '''
    Calculate the mean of a set of angles (degrees) based on polar 
    considerations.

    Parameters
    ----------
    angles: ndarray of any shape
...
def transform_XY(x : NDArray[Any], 
    ...
    x, y : ndarray of any shape
        X & Y positions
    heading : ndarray of the same shape as x & y
        Heading. The unit is specified by angle_type.
    angle_type : str (optional)
        'degree' (default) or 'radian'

    Returns
    -------
    x_rotated, y_rotated : ndarray of the same shape as x & y
...
def get_relative_pos(x : NDArray[Any]) -> NDArray[Any]:
    '''
    Get relative positions of nodes based on the center of mass (0,0).

    Parameters
    ----------
    x: nupmy array of shape (num_timepoints, N)
        Positions in a given axis. 
    
    Returns
    -------
    rel_x: ndarray of shape (num_timepoints, N)
        Positions relative to the center of mass at each time point.
    '''
    N = x.shape[1]
